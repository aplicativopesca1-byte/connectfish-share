...
function transactionCollection() {
  return db().collection("organizerWalletTransactions");
}

function summaryRef(organizerUserId: string) {
  return db().collection("organizerWalletSummaries").doc(safeTrim(organizerUserId));
}
...
async function findWalletTransactionByPaymentAndType(params: {
  ...
  const snap = await transactionCollection()
...
async function createWalletTransaction(params: {
  ...
  const ref = transactionCollection().doc();
  const timestamp = now();
  ...
  const payload = {
    ...
    createdAt: timestamp,
    releasedAt: typeof params.releasedAt === "number" ? params.releasedAt : null,
    paidOutAt: typeof params.paidOutAt === "number" ? params.paidOutAt : null,
    reversedAt: typeof params.reversedAt === "number" ? params.reversedAt : null,
    updatedAt: timestamp,
...
async function listOrganizerWalletTransactions(organizerUserId: string) {
...
export async function applyWalletPaymentReceived(input: BaseWalletEventInput) {
...
export async function applyWalletRefund(input: BaseWalletEventInput) {
  const organizerUserId = safeTrim(input.organizerUserId);
...
export async function applyWalletChargeback(input: BaseWalletEventInput) {
  const organizerUserId = safeTrim(input.organizerUserId);
...
export async function applyWalletPayoutSent(input: BaseWalletEventInput) {
  const organizerUserId = safeTrim(input.organizerUserId);
...
export async function markWalletTransactionAsPaidOut(params: {
  ...
  await transactionCollection().doc(transactionId).update({
    status: "paid_out",
    paidOutAt: now(),
    updatedAt: now(),
    serverUpdatedAt: FieldValue.serverTimestamp(),
